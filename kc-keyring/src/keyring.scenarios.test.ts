...
const scenario = {
  encrypt(algorithm: string, scenario: Scenario) {
    test(`${algorithm}: encrypts value`, (t) => {
      ...
      const krng = createKeyring(keys, { algorithm })

      const [encrypted, keyringId] = krng.encrypt(scenario.input)
  ...
  update(algorithm: string, scenario: Scenario) {
    test(`${algorithm}: updates attribute`, (t) => {
      if (!scenario.update) {
        throw new Error('scenario must have .update attribute')
      }
      ...
      let [encrypted, keyringId] = krng.encrypt(scenario.input)

      t.is(keyringId, scenario.encrypted.keyring_id)
      t.is(krng.decrypt(encrypted, keyringId), scenario.input)
      ;[encrypted, keyringId] = krng.encrypt(scenario.update.input)
      t.is(keyringId, scenario.update.encrypted.keyring_id)
      t.is(krng.decrypt(encrypted, keyringId), scenario.update.input)
    })
  },
  ...
  rotate(algorithm: string, scenario: Scenario) {
    test(`${algorithm}: rotates key`, (t) => {
      if (!scenario.rotate) {
        throw new Error('scenario must have .rotate attribute')
      }

      const keys: Record<number, string> = {}
      keys[scenario.key.id] = scenario.key.value
      let krng = createKeyring(keys, { algorithm })
      let [encrypted, keyringId] = krng.encrypt(scenario.input)
      ...
      krng = createKeyring(keys, { algorithm })
      ;[encrypted, keyringId] = krng.encrypt(scenario.input)
