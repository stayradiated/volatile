...
const UserExchangeKeysList = () => {
  const [deleteUserExchangeKeys] = useMutation<DeleteUserExchangeKeysMutation, DeleteUserExchangeKeysMutationVariables>(
    MUTATION_DELETE_USER_EXCHANGE_KEYS,
  )
  const { data, loading, error } = useQuery<GetUserExchangeKeysListQuery, GetUserExchangeKeysListQueryVariables>(
  ...
  return (
    <div>
      ...
      <Table dataSource={data?.kc_user_exchange_keys} loading={loading}>
        <Table.Column title='Description' dataIndex='description' />
        <Table.Column title='Exchange' dataIndex={['exchange', 'name']} />
        <Table.Column title='Invalidated At' dataIndex='invalidated_at' />
        <Table.Column title='DCA Order Count' dataIndex={['dca_orders_aggregate', 'aggregate', 'count']} />
        <Table.Column title='Delete' dataIndex='uid' render={(uid: string) => (
