...
test.serial('(lastRunAt=NULL, nextRunAt=PAST) set lastRunAt', async (t) => {
...
test.serial(
  '(lastRunAt=DISTANT_PAST, nextRunAt=PAST) set lastRunAt',
  async (t) => {
    const { pool, make } = t.context
...
test.serial('(lastRunAt=PAST, nextRunAt=FUTURE) do nothing', async (t) => {
  const { pool, make } = t.context
...
test.serial('(lastRunAt=FUTURE, nextRunAt=PAST) do nothing', async (t) => {
  const { pool, make } = t.context
...
test.serial(
  '(lastRunAt=DISTANT_PAST, nextRunAt=NULL) do nothing',
  async (t) => {
    const { pool, make } = t.context
